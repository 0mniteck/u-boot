...
if [ -f Builds/tee.bin ]; then
  ...
  make -j$(nproc) PLATFORM=rockchip-rk3399 CFG_ARM64_core=y CFG_STMM_PATH=BL32_AP_MM.fd CFG_RPMB_FS=y CFG_RPMB_FS_DEV_ID=1 CFG_CORE_HEAP_SIZE=524288 CFG_TZSRAM_SIZE=0x60000 CFG_RPMB_WRITE_KEY=y \
  CFG_CORE_DYN_SHM=y CFG_RPMB_TESTKEY=y CFG_REE_FS_ALLOW_RESET=y CFG_REE_FS=n CFG_CORE_ARM64_PA_BITS=48 CFG_ENABLE_EMBEDDED_TESTS=y CROSS_COMPILE=aarch64-linux-gnu- CROSS_COMPILE32=arm-linux-gnueabihf- CROSS_COMPILE_core=aarch64-linux-gnu- \
